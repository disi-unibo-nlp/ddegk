#code modified to work with graphs with two nodes and only one channel
import argparse
...
# optional arguments
parser.add_argument('--max_n_channels', type=int, default=1, help='maximum number of channels that we will be able to pass to the network')
...
def to_parallelize(file_name,p,q,dataset,path_read,path_write):
    ...
    if len(g.vs)<(max_n_channels*2): # exclude graphs with less nodes than the required min number of dims
        excluded = file_name
    try:
        emb = get_embeddings_node2vec(g,d=max_n_channels*2,p=p,q=q,path_node2vec=path_node2vec)
